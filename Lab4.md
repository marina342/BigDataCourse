...
```r
> conn <- dbConnect(RSQLite::SQLite(), "/cloud/project/database.sqlite")
...
```r
> res2 <- dbSendQuery(conn, "SELECT a.Name, p.Title FROM Authors a, Papers p WHERE a.Name='Josh Tenenbaum' ORDER BY Title")
...
```r
> res4 <- dbSendQuery(conn, "SELECT Authors.Name, COUNT(PaperAuthors.Id) AS NumPapers FROM Authors INNER JOIN PaperAuthors ON Authors.Id = PaperAuthors.AuthorId GROUP BY Authors.Name ORDER BY NumPapers desc")
